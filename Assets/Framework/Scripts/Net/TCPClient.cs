...
namespace YXCell
{
    public class SimpleTcpClient
    {
        ...
        public SimpleTcpClient(string serverIp, int serverPort)
        {
            ...
            lock (this)
            {
                try
                {
                    ...
                    if (netPacket.currRecv == NetPacket.HEADER_SIZE)
                    {
                        ...
                        socket.BeginReceive(netPacket.HeaderBytes, 0, bodySize, SocketFlags.None, ReceiveBody, netPacket);
                    }
                    else
                    {
                        //包头还没收完，继续收
            ...
            lock (this)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        socket.BeginReceive(netPacket.HeaderBytes, 0, remainSize, SocketFlags.None, ReceiveBody, netPacket);
                    }
                }
                catch (Exception)
                {
