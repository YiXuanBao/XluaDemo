...
namespace YXCell
{
    public class YXUtils
    {
        ...
        public static void InvokeAction(float seconds, Action callback, int repeatCount = 0, float repeatInterval = 1, string id = "", Action onFinish = null)
        {
            ...
            if (!YXCommonTimer.Instance) return;
            YXCommonTimer.Instance.InvokeAction(seconds, callback, repeatCount, repeatInterval, id, onFinish);
        }
        ...
        public static void InvokeActionUnlimited(float seconds, Action callback, float repeatInterval = 1, string id = "")
        {
            ...
            YXCommonTimer.Instance.InvokeActionUnlimited(seconds, callback, repeatInterval, id);
        }
        ...
        {
            YXCommonTimer.Instance.CancelInvokeAction(id);
        }
        ...
        public static Coroutine StartProcedure(YXCurve curve, float startValue, float endValue, float timeParam, Action<float> action, Action<float> onFinish = null, bool allowMultipleInstances = false, string id = "")
        {
        ...
        /// <param name="onFinish">Action called at the end of the procedure.</param>
        /// <param name="id"></param>
        public static Coroutine StartProcedure(YXCurve curve, float time, Action<float> action, Action<float> onFinish = null, string id = "")
        {
        ...
        /// <param name="onFinish">Action called at the end of the procedure.</param>
        /// <param name="id"></param>
        public static Coroutine StartProcedureUnscaled(YXCurve curve, float time, Action<float> action, Action<float> onFinish = null, string id = "")
        {
        ...
        private static IEnumerator ProcedureCR(YXCurve curve, float startValue, float endValue, float timeParam, Action<float> action, Action<float> onFinish)
        {
        ...
        private static IEnumerator ProcedureCRUnscaled(YXCurve curve, float startValue, float endValue, float timeParam, Action<float> action, Action<float> onFinish)
        {
        ...
        public static Coroutine StartCoroutine(IEnumerator cr, bool allowMultipleInstances = false, string id = "")
        {
            if (!allowMultipleInstances)
            {
                ...
                {
                    YXCommonTimer.Instance.StopCoroutine(crDict[nameof(cr)]);
                }
                crDict.InsertOrUpdate(nameof(cr), cr);
            }
            if (id.Length > 0)
                crDict.InsertOrUpdate(id, cr);
            return YXCommonTimer.Instance.StartCoroutine(cr);
        }
        ...
        {
            YXCommonTimer.Instance.StopCoroutine(cr);
        }
        /// <summary>
        /// Stops a coroutine.
        /// </summary>
        ...
        public static void StopCoroutine(string id)
        {
            if (id.Length > 0)
                if (crDict.ContainsKey(id))
                    YXCommonTimer.Instance.StopCoroutine(crDict[id]);
        }
        /// <summary>
        /// Stops a coroutine.
        /// </summary>
        ...
        {
            if (YXCommonTimer.Instance) //Batch v0.15.3: fixed issue with SKTextAnimator errors when quitting game
                YXCommonTimer.Instance.StopCoroutine(cr);
        }
        #endregion
    }
